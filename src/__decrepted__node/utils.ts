import { MetaProps, Query } from './ICore';
 ...
 * @return {MetaProps}
 */
export function parseMeta(metaString: string): MetaProps {
    const meta: Record<string, string> = {};

    metaString.split(' ').forEach(str => {
        const [key, ...rest] = str.split('=', 2);
        const value = rest.length === 1 ? rest[0] : rest.join('=');
 ...
 */
export function mergeQuery(baseQuery: Query, ...queries: Query[]): URLSearchParams {
 ...
 * @return {string}
 */
export function mergeStyle(baseStyle: string, style: string): string {
    ...
    const mergedObject = Object.fromEntries(baseStyleEntries.concat(styleEntries));
